...
public class DigestScheme extends RFC2617Scheme {
    ...
    private String createDigest(Credentials credentials) throws AuthenticationException {
        ...
        tmp.append(':');
        tmp.append(pwd);
        String a12 = tmp.toString();
        if (algorithm.equalsIgnoreCase("MD5-sess")) {
            String cnonce = getCnonce();
            String tmp2 = encode(md5Helper.digest(EncodingUtils.getBytes(a12, charset)));
        ...
        String md5a1 = encode(md5Helper.digest(EncodingUtils.getBytes(a12, charset)));
        String a22 = null;
        if (this.qopVariant != 1) {
            a22 = method + ':' + uri;
        }
        String md5a2 = encode(md5Helper.digest(EncodingUtils.getAsciiBytes(a22)));
    ...
    private Header createDigestHeader(Credentials credentials, String digest) throws AuthenticationException {
        ...
        params.add(new BasicNameValuePair(HintConstants.AUTOFILL_HINT_USERNAME, uname));
        ...
        for (int i5 = 0; i5 < params.size(); i5++) {
            BasicNameValuePair param = params.get(i5);
            if (i5 > 0) {
                buffer.append(", ");
            }
            boolean z4 = true;
            ...
            basicHeaderValueFormatter.formatNameValuePair(buffer, param, z4);
        }
    ...
    private static String encode(byte[] binaryData) {
        ...
        for (int i5 = 0; i5 < 16; i5++) {
            int low = binaryData[i5] & 15;
            int high = (binaryData[i5] & 240) >> 4;
            buffer[i5 * 2] = HEXADECIMAL[high];
            buffer[(i5 * 2) + 1] = HEXADECIMAL[low];
        }
