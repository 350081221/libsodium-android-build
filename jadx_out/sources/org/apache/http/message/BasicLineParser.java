...
public class BasicLineParser implements LineParser {
    ...
    public ProtocolVersion parseProtocolVersion(CharArrayBuffer buffer, ParserCursor cursor) throws ParseException {
        ...
        int i5 = cursor.getPos();
        if (i5 + protolength + 4 > indexTo) {
        ...
        while (true) {
            boolean z4 = true;
            if (!ok || j5 >= protolength) {
                break;
            }
            if (buffer.charAt(i5 + j5) != protoname.charAt(j5)) {
                z4 = false;
            }
            ok = z4;
            j5++;
        ...
        if (ok) {
            ok = buffer.charAt(i5 + protolength) == '/';
        }
        if (!ok) {
            throw new ParseException("Not a valid protocol version: " + buffer.substring(indexFrom, indexTo));
        }
        int i6 = i5 + protolength + 1;
        int period = buffer.indexOf(46, i6, indexTo);
        if (period == -1) {
            throw new ParseException("Invalid protocol version number: " + buffer.substring(indexFrom, indexTo));
        }
        try {
            int major = Integer.parseInt(buffer.substringTrimmed(i6, period));
            int i7 = period + 1;
            int blank = buffer.indexOf(32, i7, indexTo);
            if (blank == -1) {
                blank = indexTo;
            }
            try {
                int minor = Integer.parseInt(buffer.substringTrimmed(i7, blank));
    ...
    public boolean hasProtocolVersion(CharArrayBuffer buffer, ParserCursor cursor) {
        ...
        while (true) {
            ...
            if (buffer.charAt(index + j5) != protoname.charAt(j5)) {
                z4 = false;
            }
            ok = z4;
            j5++;
    ...
    public RequestLine parseRequestLine(CharArrayBuffer buffer, ParserCursor cursor) throws ParseException {
        ...
        try {
            skipWhitespace(buffer, cursor);
            int i5 = cursor.getPos();
            int blank = buffer.indexOf(32, i5, indexTo);
            if (blank < 0) {
                throw new ParseException("Invalid request line: " + buffer.substring(indexFrom, indexTo));
            }
            String method = buffer.substringTrimmed(i5, blank);
            cursor.updatePos(blank);
            skipWhitespace(buffer, cursor);
            int i6 = cursor.getPos();
            int blank2 = buffer.indexOf(32, i6, indexTo);
            if (blank2 < 0) {
                throw new ParseException("Invalid request line: " + buffer.substring(indexFrom, indexTo));
            }
            String uri = buffer.substringTrimmed(i6, blank2);
    ...
    public StatusLine parseStatusLine(CharArrayBuffer buffer, ParserCursor cursor) throws ParseException {
        ...
        try {
            ...
            try {
                int statusCode = Integer.parseInt(buffer.substringTrimmed(i5, blank));
                int i6 = blank;
                if (i6 < indexTo) {
                    reasonPhrase = buffer.substringTrimmed(i6, indexTo);
                } else {
                    reasonPhrase = "";
                }
