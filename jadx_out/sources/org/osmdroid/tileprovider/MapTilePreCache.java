    ...
    private void refresh() {
        ...
        synchronized (this.mTileAreas) {
            ...
            for (MapTileArea mapTileArea2 : this.mCache.getAdditionalMapTileList().getList()) {
                if (i5 < this.mTileAreas.getList().size()) {
                    mapTileArea = this.mTileAreas.getList().get(i5);
                } else {
            ...
            while (i5 < this.mTileAreas.getList().size()) {
    ...
    public void search(long j5) {
        for (MapTileModuleProviderBase mapTileModuleProviderBase : this.mProviders) {
            if (mapTileModuleProviderBase instanceof MapTileDownloader) {
                e tileSource = ((MapTileDownloader) mapTileModuleProviderBase).getTileSource();
                if ((tileSource instanceof OnlineTileSourceBase) && !((OnlineTileSourceBase) tileSource).getTileSourcePolicy().c()) {
                }
            }
            Drawable loadTileIfReachable = mapTileModuleProviderBase.getTileLoader().loadTileIfReachable(j5);
            if (loadTileIfReachable != null) {
                this.mCache.putTile(j5, loadTileIfReachable);
                return;
            }
        }
    }
