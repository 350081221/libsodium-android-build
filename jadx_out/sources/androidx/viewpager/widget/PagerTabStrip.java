...
public class PagerTabStrip extends PagerTitleStrip {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float f5 = height;
        canvas.drawRect(left, i5, right, f5, this.mTabPaint);
        if (this.mDrawFullUnderline) {
            this.mTabPaint.setColor((this.mIndicatorColor & ViewCompat.MEASURED_SIZE_MASK) | ViewCompat.MEASURED_STATE_MASK);
            canvas.drawRect(getPaddingLeft(), height - this.mFullUnderlineHeight, getWidth() - getPaddingRight(), f5, this.mTabPaint);
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        if (action != 0) {
            if (action != 1) {
                if (action == 2 && (Math.abs(x4 - this.mInitialMotionX) > this.mTouchSlop || Math.abs(y4 - this.mInitialMotionY) > this.mTouchSlop)) {
                    this.mIgnoreTap = true;
                }
            } else if (x4 < this.mCurrText.getLeft() - this.mTabPadding) {
                ViewPager viewPager = this.mPager;
                viewPager.setCurrentItem(viewPager.getCurrentItem() - 1);
            } else if (x4 > this.mCurrText.getRight() + this.mTabPadding) {
