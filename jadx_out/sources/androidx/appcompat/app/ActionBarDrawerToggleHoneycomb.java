...
class ActionBarDrawerToggleHoneycomb {
    ...
    public static SetIndicatorInfo setActionBarDescription(SetIndicatorInfo setIndicatorInfo, Activity activity, int i5) {
        if (setIndicatorInfo == null) {
            setIndicatorInfo = new SetIndicatorInfo(activity);
        }
        if (setIndicatorInfo.setHomeAsUpIndicator != null) {
            try {
                setIndicatorInfo.setHomeActionContentDescription.invoke(activity.getActionBar(), Integer.valueOf(i5));
            } catch (Exception e5) {
                Log.w(TAG, "Couldn't set content description via JB-MR2 API", e5);
            }
        }
        return setIndicatorInfo;
    }

    public static SetIndicatorInfo setActionBarUpIndicator(Activity activity, Drawable drawable, int i5) {
        ...
        if (setIndicatorInfo.setHomeAsUpIndicator != null) {
            try {
                ...
                setIndicatorInfo.setHomeActionContentDescription.invoke(actionBar, Integer.valueOf(i5));
            } catch (Exception e5) {
                Log.w(TAG, "Couldn't set home-as-up indicator via JB-MR2 API", e5);
            }
        } else {
