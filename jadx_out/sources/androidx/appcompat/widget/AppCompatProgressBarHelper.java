...
class AppCompatProgressBarHelper {
    ...
    private Drawable tileifyIndeterminate(Drawable drawable) {
        if (drawable instanceof AnimationDrawable) {
            ...
            for (int i5 = 0; i5 < numberOfFrames; i5++) {
                Drawable tileify = tileify(animationDrawable.getFrame(i5), true);
                tileify.setLevel(10000);
                animationDrawable2.addFrame(tileify, animationDrawable.getDuration(i5));
            }
    ...
    public void loadFromAttributes(AttributeSet attributeSet, int i5) {
        TintTypedArray obtainStyledAttributes = TintTypedArray.obtainStyledAttributes(this.mView.getContext(), attributeSet, TINT_ATTRS, i5, 0);
    ...
    @VisibleForTesting
    Drawable tileify(Drawable drawable, boolean z4) {
        boolean z5;
        if (drawable instanceof WrappedDrawable) {
            WrappedDrawable wrappedDrawable = (WrappedDrawable) drawable;
            Drawable wrappedDrawable2 = wrappedDrawable.getWrappedDrawable();
            if (wrappedDrawable2 != null) {
                wrappedDrawable.setWrappedDrawable(tileify(wrappedDrawable2, z4));
            }
        } else {
            if (drawable instanceof LayerDrawable) {
                ...
                for (int i5 = 0; i5 < numberOfLayers; i5++) {
                    ...
                    drawableArr[i5] = tileify(drawable2, z5);
                }
