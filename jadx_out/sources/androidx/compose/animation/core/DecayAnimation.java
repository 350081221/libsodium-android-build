...
public final class DecayAnimation<T, V extends AnimationVector> implements Animation<T, V> {
    ...
    public DecayAnimation(@l VectorizedDecayAnimationSpec<V> vectorizedDecayAnimationSpec, @l TwoWayConverter<T, V> twoWayConverter, T t5, @l V v4) {
        float H;
        ...
        this.targetValue = getTypeConverter().getConvertFromVector().invoke(vectorizedDecayAnimationSpec.getTargetValue(invoke, v4));
        this.durationNanos = vectorizedDecayAnimationSpec.getDurationNanos(invoke, v4);
        V v5 = (V) AnimationVectorsKt.copy(vectorizedDecayAnimationSpec.getVelocityFromNanos(getDurationNanos(), invoke, v4));
        this.endVelocity = v5;
        ...
        for (int i5 = 0; i5 < size$animation_core_release; i5++) {
            ...
            H = u.H(v6.get$animation_core_release(i5), -this.animationSpec.getAbsVelocityThreshold(), this.animationSpec.getAbsVelocityThreshold());
            v6.set$animation_core_release(i5, H);
        }
    }
    ...
    public T getValueFromNanos(long j5) {
        if (!isFinishedFromNanos(j5)) {
            return (T) getTypeConverter().getConvertFromVector().invoke(this.animationSpec.getValueFromNanos(j5, this.initialValueVector, this.initialVelocityVector));
        }
    ...
    public V getVelocityVectorFromNanos(long j5) {
        if (!isFinishedFromNanos(j5)) {
            return this.animationSpec.getVelocityFromNanos(j5, this.initialValueVector, this.initialVelocityVector);
        }
    ...
    public DecayAnimation(@l DecayAnimationSpec<T> decayAnimationSpec, @l TwoWayConverter<T, V> twoWayConverter, T t5, @l V v4) {
        this(decayAnimationSpec.vectorize(twoWayConverter), twoWayConverter, t5, v4);
    }

    public DecayAnimation(@l DecayAnimationSpec<T> decayAnimationSpec, @l TwoWayConverter<T, V> twoWayConverter, T t5, T t6) {
        this(decayAnimationSpec.vectorize(twoWayConverter), twoWayConverter, t5, twoWayConverter.getConvertToVector().invoke(t6));
    }
}
