...
public final class Cam16 {
    ...
    private final double f7316j;
    private final double jstar;

    /* renamed from: m, reason: collision with root package name */
    private final double f7317m;

    /* renamed from: q, reason: collision with root package name */
    private final double f7318q;

    /* renamed from: s, reason: collision with root package name */
    private final double f7319s;
    ...
    private static Cam16 fromJchInViewingConditions(double d5, double d6, double d7, ViewingConditions viewingConditions) {
        ...
        return new Cam16(d7, d6, d5, c5, flRoot, sqrt, d9, Math.cos(radians) * log1p, Math.sin(radians) * log1p);
    }
    ...
    public static Cam16 fromXyzInViewingConditions(double d5, double d6, double d7, ViewingConditions viewingConditions) {
        double d8;
        ...
        double[] dArr4 = dArr[2];
        double d11 = (dArr4[0] * d5) + (dArr4[1] * d6) + (dArr4[2] * d7);
        ...
        double c5 = (4.0d / viewingConditions.getC()) * Math.sqrt(d21) * (viewingConditions.getAw() + 4.0d) * viewingConditions.getFlRoot();
        if (d20 < 20.14d) {
            d8 = d20 + 360.0d;
        } else {
            d8 = d20;
        }
        double pow5 = Math.pow(1.64d - Math.pow(0.29d, viewingConditions.getN()), 0.73d) * Math.pow(((((((Math.cos(Math.toRadians(d8) + 2.0d) + 3.8d) * 0.25d) * 3846.153846153846d) * viewingConditions.getNc()) * viewingConditions.getNcb()) * Math.hypot(d15, d16)) / (d18 + 0.305d), 0.9d);
        ...
        return new Cam16(d20, sqrt, pow4, c5, flRoot, Math.sqrt((pow5 * viewingConditions.getC()) / (viewingConditions.getAw() + 4.0d)) * 50.0d, (1.7000000000000002d * pow4) / ((0.007d * pow4) + 1.0d), log1p * Math.cos(radians), log1p * Math.sin(radians));
    }
