...
public class HctSolver {
    ...
    static boolean areInCyclicOrder(double d5, double d6, double d7) {
        if (sanitizeRadians(d6 - d5) < sanitizeRadians(d7 - d5)) {
            return true;
        }
        return false;
    }

    static double[] bisectToLimit(double d5, double d6) {
        int criticalPlaneAbove;
        int criticalPlaneBelow;
        double[][] bisectToSegment = bisectToSegment(d5, d6);
        ...
        for (int i5 = 0; i5 < 3; i5++) {
            ...
            if (d7 != d8) {
                if (d7 < d8) {
                    criticalPlaneAbove = criticalPlaneBelow(trueDelinearized(d7));
                    criticalPlaneBelow = criticalPlaneAbove(trueDelinearized(dArr2[i5]));
                } else {
                    criticalPlaneAbove = criticalPlaneAbove(trueDelinearized(d7));
    ...
    static double[][] bisectToSegment(double d5, double d6) {
        ...
        for (int i5 = 0; i5 < 12; i5++) {
            double[] nthVertex = nthVertex(d5, i5);
            if (nthVertex[0] >= 0.0d) {
                ...
                if (!z4) {
                    ...
                    d7 = hueOf;
                    d8 = d7;
                } else if (z5 || areInCyclicOrder(d7, hueOf, d8)) {
                    if (areInCyclicOrder(d7, d6, hueOf)) {
                        z5 = false;
                        dArr2 = nthVertex;
                        d8 = hueOf;
                    } else {
    ...
    static double chromaticAdaptation(double d5) {
        double pow = Math.pow(Math.abs(d5), 0.42d);
        return ((MathUtils.signum(d5) * 400.0d) * pow) / (pow + 27.13d);
    }

    static int criticalPlaneAbove(double d5) {
        return (int) Math.ceil(d5 - 0.5d);
    }

    static int criticalPlaneBelow(double d5) {
        return (int) Math.floor(d5 - 0.5d);
    }

    static int findResultByJ(double d5, double d6, double d7) {
        double d8;
        double sqrt = Math.sqrt(d7) * 11.0d;
        ViewingConditions viewingConditions = ViewingConditions.DEFAULT;
        double pow = 1.0d / Math.pow(1.64d - Math.pow(0.29d, viewingConditions.getN()), 0.73d);
        double cos = (Math.cos(d5 + 2.0d) + 3.8d) * 0.25d * 3846.153846153846d * viewingConditions.getNc() * viewingConditions.getNcb();
        ...
        while (i5 < 5) {
            ...
            double d10 = d8 * pow;
            double d11 = pow;
            double pow2 = Math.pow(d10, 1.1111111111111112d);
            double d12 = sqrt;
    ...
    static double intercept(double d5, double d6, double d7) {
        return (d6 - d5) / (d7 - d5);
    }

    static double inverseChromaticAdaptation(double d5) {
        double abs = Math.abs(d5);
        return MathUtils.signum(d5) * Math.pow(Math.max(0.0d, (27.13d * abs) / (400.0d - abs)), 2.380952380952381d);
    }

    static boolean isBounded(double d5) {
        return 0.0d <= d5 && d5 <= 100.0d;
    }

    static double[] lerpPoint(double[] dArr, double d5, double[] dArr2) {
        double d6 = dArr[0];
        double d7 = dArr[1];
        double d8 = dArr[2];
        return new double[]{d6 + ((dArr2[0] - d6) * d5), d7 + ((dArr2[1] - d7) * d5), d8 + ((dArr2[2] - d8) * d5)};
    }
    ...
    static double[] nthVertex(double d5, int i5) {
        double d6;
        double[] dArr = Y_FROM_LINRGB;
        double d7 = dArr[0];
        double d8 = dArr[1];
        ...
        if (i5 < 4) {
            double d11 = ((d5 - (d8 * d6)) - (d9 * d10)) / d7;
            if (isBounded(d11)) {
                return new double[]{d11, d6, d10};
            }
            return new double[]{-1.0d, -1.0d, -1.0d};
        }
        if (i5 < 8) {
            double d12 = ((d5 - (d7 * d10)) - (d9 * d6)) / d8;
            if (isBounded(d12)) {
                return new double[]{d10, d12, d6};
            }
            return new double[]{-1.0d, -1.0d, -1.0d};
        }
        double d13 = ((d5 - (d7 * d6)) - (d8 * d10)) / d9;
        if (isBounded(d13)) {
            return new double[]{d6, d10, d13};
        }
        return new double[]{-1.0d, -1.0d, -1.0d};
    }

    static double sanitizeRadians(double d5) {
        return (d5 + 25.132741228718345d) % 6.283185307179586d;
    }

    static double[] setCoordinate(double[] dArr, double d5, double[] dArr2, int i5) {
        return lerpPoint(dArr, intercept(dArr[i5], d5, dArr2[i5]), dArr2);
    }

    public static Cam16 solveToCam(double d5, double d6, double d7) {
        return Cam16.fromInt(solveToInt(d5, d6, d7));
    }

    public static int solveToInt(double d5, double d6, double d7) {
        if (d6 >= 1.0E-4d && d7 >= 1.0E-4d && d7 <= 99.9999d) {
            double sanitizeDegreesDouble = (MathUtils.sanitizeDegreesDouble(d5) / 180.0d) * 3.141592653589793d;
            double yFromLstar = ColorUtils.yFromLstar(d7);
            int findResultByJ = findResultByJ(sanitizeDegreesDouble, d6, yFromLstar);
            if (findResultByJ != 0) {
    ...
    static double trueDelinearized(double d5) {
        double d6 = d5 / 100.0d;
