...
public class FailedBinderCallBack {
    ...
    private void agingCheck() {
        ...
        for (Long l5 : binderCallBackMap.keySet()) {
            if (time >= l5.longValue()) {
                binderCallBackMap.remove(l5);
            }
        }
    }
    ...
    private void putCallBackInMap(Long l5, BinderCallBack binderCallBack) {
        if (binderCallBackMap == null) {
            HMSLog.e(TAG, "binderCallBackMap is null");
        } else {
            agingCheck();
            binderCallBackMap.put(l5, binderCallBack);
        }
    }

    public BinderCallBack getCallBack(Long l5) {
        Map<Long, BinderCallBack> map = binderCallBackMap;
        if (map == null) {
            HMSLog.e(TAG, "binderCallBackMap is null");
            return null;
        }
        return map.remove(l5);
    }

    public void setCallBack(Long l5, BinderCallBack binderCallBack) {
        putCallBackInMap(l5, binderCallBack);
    }
}
