...
public final class SafeParcelableSerializer {
    ...
    public static <S extends SafeParcelable> ArrayList<S> deserializeIterableFromBundle(Bundle bundle, String str, Parcelable.Creator<S> creator) {
        ...
        w2.o oVar = (ArrayList<S>) new ArrayList(arrayList.size());
        Iterator it = arrayList.iterator();
        while (it.hasNext()) {
            oVar.add(deserializeFromBytes((byte[]) it.next(), creator));
        }
        return oVar;
    }
    ...
    public static <S extends SafeParcelable> byte[] serializeToBytes(S s5) {
        Parcel obtain = Parcel.obtain();
        s5.writeToParcel(obtain, 0);
        byte[] marshall = obtain.marshall();
        obtain.recycle();
        return marshall;
    }

    public static <S extends SafeParcelable> void serializeToIntentExtra(S s5, Intent intent, String str) {
        intent.putExtra(str, serializeToBytes(s5));
    }

    public static <S extends SafeParcelable> String serializeToString(S s5) {
        return Base64Utils.encodeUrlSafe(serializeToBytes(s5));
    }
}
