...
public class HianalyticsHelper {
    ...
    private void onNewEvent(Context context, String str, Map map, int i5) {
        if (context != null && map != null) {
            Logger.v(TAG, "data = %s", map);
            try {
                HiAnalyticsUtils.getInstance().onNewEvent(context, str, map, i5);
    ...
    public void reportData(Context context, LinkedHashMap<String, String> linkedHashMap, String str, int i5) {
        if (!isEnableReportNoSeed(context)) {
            return;
        }
        onEvent(linkedHashMap, str, i5);
    }
    ...
    public void onEvent(LinkedHashMap<String, String> linkedHashMap, String str, int i5) {
        HiAnalyticsInstance hiAnalyticsInstance;
        if (this.hasHMSBI) {
            onNewEvent(ContextHolder.getAppContext(), str, linkedHashMap, i5);
        } else if (i5 == 0) {
            Logger.v(TAG, "the base sdk isn't exsit, and reportType is %s", Integer.valueOf(i5));
            return;
        }
