...
public class MemoryRegionParser {
    ...
    public static List<MappedMemoryRegion> getMemoryRegions(int i5) throws IOException {
        LinkedList linkedList = new LinkedList();
        BufferedReader bufferedReader = new BufferedReader(new FileReader(String.format(Locale.ENGLISH, "/proc/%d/maps", Integer.valueOf(i5))));
        while (true) {
    ...
    private static MappedMemoryRegion parseMapLine(String str) {
        ...
        if (matcher.matches()) {
            if (matcher.groupCount() == 11) {
                return new MappedMemoryRegion(parseHex(matcher.group(1)), parseHex(matcher.group(2)), matcher.group(3).equals("r"), matcher.group(4).equals("w"), matcher.group(5).equals("x"), matcher.group(6).equals(bi.aE), parseHex(matcher.group(7)), parseHex(matcher.group(8)), parseHex(matcher.group(9)), parseHex(matcher.group(10)), matcher.group(11));
            }
