...
public class ClassReferenceListBuilder {
    ...
    private void addClassWithHierachy(String str) {
        ...
        try {
            ...
            for (int i5 = 0; i5 < size; i5++) {
                addClassWithHierachy(interfaces.getType(i5).getClassName());
            }
    ...
    private void addDependencies(DirectClassFile directClassFile) {
        ...
        for (int i5 = 0; i5 < size; i5++) {
            checkDescriptor(fields.get(i5).getDescriptor().getString());
        }
        MethodList methods = directClassFile.getMethods();
        int size2 = methods.size();
        for (int i6 = 0; i6 < size2; i6++) {
            checkPrototype(Prototype.intern(methods.get(i6).getDescriptor().getString()));
        }
    }

    private void checkDescriptor(String str) {
        if (str.endsWith(i.f136b)) {
    ...
    private void checkPrototype(Prototype prototype) {
        ...
        for (int i5 = 0; i5 < parameterTypes.size(); i5++) {
            checkDescriptor(parameterTypes.get(i5).getDescriptor());
        }
    }
        ...
        while (entries2.hasMoreElements()) {
            ...
            if (name2.endsWith(CLASS_EXTENSION)) {
                try {
                    ...
                    throw new IOException("Class " + name2 + " is missing form original class path " + this.path, e5);
                }
            }
        }
    }
