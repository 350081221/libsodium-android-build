...
public final class BasicBlock implements LabeledItem {
    ...
    public BasicBlock(int i5, InsnList insnList, IntList intList, int i6) {
        if (i5 >= 0) {
            try {
                ...
                if (size != 0) {
                    for (int i7 = size - 2; i7 >= 0; i7--) {
                        if (insnList.get(i7).getOpcode().getBranchingness() != 1) {
                    ...
                    if (insnList.get(size - 1).getOpcode().getBranchingness() != 1) {
                        try {
                            ...
                            if (i6 >= -1) {
                                if (i6 >= 0 && !intList.contains(i6)) {
                                    throw new IllegalArgumentException("primarySuccessor " + i6 + " not in successors " + intList);
                                }
                                this.label = i5;
                                this.insns = insnList;
    ...
    public int getSecondarySuccessor() {
        if (this.successors.size() == 2) {
            int i5 = this.successors.get(0);
            if (i5 == this.primarySuccessor) {
    ...
    public BasicBlock withRegisterOffset(int i5) {
        return new BasicBlock(this.label, this.insns.withRegisterOffset(i5), this.successors, this.primarySuccessor);
    }
}
