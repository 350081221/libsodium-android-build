...
public class EscapeAnalysis {
    ...
    public static class EscapeSet {
        ...
        EscapeSet(int i5, int i6, EscapeState escapeState) {
    ...
    private int findSetIndex(RegisterSpec registerSpec) {
        int i5 = 0;
        while (i5 < this.latticeValues.size()) {
            if (this.latticeValues.get(i5).regSet.get(registerSpec.getReg())) {
                return i5;
            }
            i5++;
        }
    ...
    private void insertPlainInsnBefore(SsaInsn ssaInsn, RegisterSpecList registerSpecList, RegisterSpec registerSpec, int i5, Constant constant) {
        Rop ropFor;
        Insn plainCstInsn;
        ...
        if (i5 == 56) {
            ...
            ropFor = Rops.ropFor(i5, registerSpec, registerSpecList, constant);
        }
        Rop rop = ropFor;
        if (constant == null) {
    ...
    private void insertThrowingInsnBefore(SsaInsn ssaInsn, RegisterSpecList registerSpecList, RegisterSpec registerSpec, int i5, Constant constant) {
        Insn throwingCstInsn;
        Insn originalRopInsn = ssaInsn.getOriginalRopInsn();
        Rop ropFor = Rops.ropFor(i5, registerSpec, registerSpecList, constant);
        if (constant == null) {
    ...
    private void movePropagate() {
        for (int i5 = 0; i5 < this.ssaMeth.getRegCount(); i5++) {
    ...
    private void replaceDef(SsaInsn ssaInsn, SsaInsn ssaInsn2, int i5, ArrayList<RegisterSpec> arrayList) {
    ...
    private void replaceUse(SsaInsn ssaInsn, SsaInsn ssaInsn2, ArrayList<RegisterSpec> arrayList, HashSet<SsaInsn> hashSet) {
        ...
        if (opcode != 34) {
            ...
            for (int i5 = 0; i5 < size; i5++) {
                RegisterSpec make = RegisterSpec.make(arrayList.get(i5).getReg(), (TypeBearer) initValues.get(i5));
                insertPlainInsnBefore(ssaInsn, RegisterSpecList.EMPTY, make, 5, initValues.get(i5));
                arrayList.set(i5, make);
            }
            return;
        }
