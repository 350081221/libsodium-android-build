...
public class AnnotationLister {
    ...
    public void printMatch(DirectClassFile directClassFile) {
        ...
        while (it.hasNext()) {
            int i5 = AnonymousClass2.$SwitchMap$com$android$dx$command$annotool$Main$PrintType[((Main.PrintType) it.next()).ordinal()];
            if (i5 != 1) {
                if (i5 == 2) {
                    this.matchInnerClassesOf.add(directClassFile.getThisClass().getClassType().getClassName());
                }
            } else {
                System.out.println(directClassFile.getThisClass().getClassType().getClassName().replace(ClassPathElement.SEPARATOR_CHAR, d.f15957a));
            }
        }
    }
    ...
    private void printMatchPackage(String str) {
        ...
        while (it.hasNext()) {
            ...
            if (i5 != 1 && i5 != 2 && i5 != 3) {
                if (i5 == 4) {
                    System.out.println(str.replace(ClassPathElement.SEPARATOR_CHAR, d.f15957a));
                }
            } else {
    ...
    public void process() {
        for (String str : this.args.files) {
            new ClassPathOpener(str, true, new ClassPathOpener.Consumer() { // from class: com.android.dx.command.annotool.AnnotationLister.1
                ...
                @Override // com.android.dx.cf.direct.ClassPathOpener.Consumer
                public boolean processFileBytes(String str2, long j5, byte[] bArr) {
