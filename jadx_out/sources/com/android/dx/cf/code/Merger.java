...
public final class Merger {
    ...
    public static boolean isPossiblyAssignableFrom(TypeBearer typeBearer, TypeBearer typeBearer2) {
        ...
        if (basicType == 9 && basicType2 == 9) {
            ...
            if (!type2.isArray() || type == Type.SERIALIZABLE || type == Type.CLONEABLE) {
                return true;
            }
            return false;
        }
    ...
    public static OneLocalsArray mergeLocals(OneLocalsArray oneLocalsArray, OneLocalsArray oneLocalsArray2) {
        ...
        if (oneLocalsArray2.getMaxLocals() == maxLocals) {
            ...
            for (int i5 = 0; i5 < maxLocals; i5++) {
                TypeBearer orNull = oneLocalsArray.getOrNull(i5);
                TypeBearer mergeType = mergeType(orNull, oneLocalsArray2.getOrNull(i5));
                if (mergeType != orNull) {
                    if (oneLocalsArray3 == null) {
                        oneLocalsArray3 = oneLocalsArray.copy();
                    }
                    if (mergeType == null) {
                        oneLocalsArray3.invalidate(i5);
                    } else {
                        oneLocalsArray3.set(i5, mergeType);
                    }
                }
            }
            if (oneLocalsArray3 == null) {
    ...
    public static ExecutionStack mergeStack(ExecutionStack executionStack, ExecutionStack executionStack2) {
        ...
        if (executionStack2.size() == size) {
            ...
            for (int i5 = 0; i5 < size; i5++) {
                TypeBearer peek = executionStack.peek(i5);
                TypeBearer peek2 = executionStack2.peek(i5);
                ...
                if (mergeType != peek) {
                    ...
                    if (mergeType != null) {
                        try {
                            executionStack3.change(i5, mergeType);
                        } catch (SimException e5) {
                            e5.addContext("...while merging stack[" + Hex.u2(i5) + "]");
                            throw e5;
                        }
                    } else {
