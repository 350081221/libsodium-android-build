...
public class LocalsArraySet extends LocalsArray {
    ...
    public LocalsArraySet(int i5) {
        super(i5 != 0);
        this.primary = new OneLocalsArray(i5);
        this.secondaries = new ArrayList<>();
    }

    private LocalsArray getSecondaryForLabel(int i5) {
        if (i5 >= this.secondaries.size()) {
            return null;
        }
        return this.secondaries.get(i5);
    }
    ...
    private LocalsArraySet mergeWithOne(OneLocalsArray oneLocalsArray) {
        ...
        for (int i5 = 0; i5 < size; i5++) {
            LocalsArray localsArray = this.secondaries.get(i5);
            if (localsArray != null) {
                try {
                    merge = localsArray.merge(oneLocalsArray);
                } catch (SimException e5) {
                    e5.addContext("Merging one locals against caller block " + Hex.u2(i5));
                }
                if (z4 && localsArray == merge) {
                    z4 = false;
                } else {
                    z4 = true;
                }
    ...
    private LocalsArraySet mergeWithSet(LocalsArraySet localsArraySet) {
        ...
        for (int i5 = 0; i5 < max; i5++) {
            ...
            if (i5 < size) {
                localsArray = this.secondaries.get(i5);
            } else {
                localsArray = null;
            }
            if (i5 < size2) {
                localsArray2 = localsArraySet.secondaries.get(i5);
            } else {
                localsArray2 = null;
            }
            if (localsArray != localsArray2) {
                if (localsArray == null) {
                    ...
                    try {
                        ...
                        e5.addContext("Merging locals set for caller block " + Hex.u2(i5));
                    }
                }
                if (z4 && localsArray == localsArray3) {
                    z4 = false;
                } else {
                    z4 = true;
                }
    ...
    public void annotate(ExceptionWithContext exceptionWithContext) {
        ...
        for (int i5 = 0; i5 < size; i5++) {
            LocalsArray localsArray = this.secondaries.get(i5);
            if (localsArray != null) {
                exceptionWithContext.addContext("(locals array set: primary for caller " + Hex.u2(i5) + ')');
    ...
    public TypeBearer get(int i5) {
        return this.primary.get(i5);
    }

    @Override // com.android.dx.cf.code.LocalsArray
    public TypeBearer getCategory1(int i5) {
        return this.primary.getCategory1(i5);
    }

    @Override // com.android.dx.cf.code.LocalsArray
    public TypeBearer getCategory2(int i5) {
        return this.primary.getCategory2(i5);
    }
    ...
    public TypeBearer getOrNull(int i5) {
        return this.primary.getOrNull(i5);
    }
    ...
    public LocalsArraySet mergeWithSubroutineCaller(LocalsArray localsArray, int i5) {
        LocalsArray localsArray2;
        LocalsArray secondaryForLabel = getSecondaryForLabel(i5);
        ...
        for (int i6 = 0; i6 < max; i6++) {
            if (i6 == i5) {
                ...
                localsArray2 = this.secondaries.get(i6);
            } else {
                localsArray2 = null;
            }
    ...
    public void set(int i5, TypeBearer typeBearer) {
        throwIfImmutable();
        this.primary.set(i5, typeBearer);
        Iterator<LocalsArray> it = this.secondaries.iterator();
        while (it.hasNext()) {
            LocalsArray next = it.next();
            if (next != null) {
                next.set(i5, typeBearer);
            }
        }
    }
    ...
    public LocalsArray subArrayForLabel(int i5) {
        return getSecondaryForLabel(i5);
    }
    ...
    public String toHuman() {
        ...
        for (int i5 = 0; i5 < size; i5++) {
            LocalsArray localsArray = this.secondaries.get(i5);
            if (localsArray != null) {
                sb.append("(locals array set: primary for caller " + Hex.u2(i5) + ")\n");
    ...
    private LocalsArraySet(LocalsArraySet localsArraySet) {
        ...
        for (int i5 = 0; i5 < size; i5++) {
            LocalsArray localsArray = localsArraySet.secondaries.get(i5);
            if (localsArray == null) {
