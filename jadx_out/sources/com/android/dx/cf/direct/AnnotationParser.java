...
public final class AnnotationParser {
    ...
    public AnnotationParser(DirectClassFile directClassFile, int i5, int i6, ParseObserver parseObserver) {
        if (directClassFile != null) {
            this.cf = directClassFile;
            this.pool = directClassFile.getConstantPool();
            this.observer = parseObserver;
            ByteArray slice = directClassFile.getBytes().slice(i5, i6 + i5);
            this.bytes = slice;
    ...
    private void changeIndent(int i5) {
        this.observer.changeIndent(i5);
    }
    ...
    private AnnotationsList parseAnnotationsList(AnnotationVisibility annotationVisibility) throws IOException {
        ...
        for (int i5 = 0; i5 < readUnsignedByte; i5++) {
            if (this.observer != null) {
                parsed(0, "parameter_annotations[" + i5 + "]:");
                changeIndent(1);
            }
            annotationsList.set(i5, parseAnnotations(annotationVisibility));
            ParseObserver parseObserver = this.observer;
            if (parseObserver != null) {
                parseObserver.changeIndent(-1);
            }
                                    ...
                                    if (readUnsignedByte != 74) {
                                        if (readUnsignedByte != 90) {
                                            ...
                                            for (int i5 = 0; i5 < readUnsignedShort; i5++) {
                                                if (this.observer != null) {
                                                    changeIndent(-1);
                                                    parsed(0, "element_value[" + i5 + "]:");
                                                    changeIndent(1);
                                                }
                                                list.set(i5, parseValue());
                                            }
                                            if (this.observer != null) {
                                                changeIndent(-1);
                                            }
    ...
    private void parsed(int i5, String str) {
        this.observer.parsed(this.bytes, this.parseCursor, i5, str);
        this.parseCursor += i5;
    }

    private void requireLength(int i5) throws IOException {
        if (this.input.available() >= i5) {
        } else {
