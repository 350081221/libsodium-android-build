...
public final class AttributeListParser {
    ...
    public AttributeListParser(DirectClassFile directClassFile, int i5, int i6, AttributeFactory attributeFactory) {
        if (directClassFile != null) {
            if (attributeFactory != null) {
                int unsignedShort = directClassFile.getBytes().getUnsignedShort(i6);
        ...
        for (int i6 = 0; i6 < size; i6++) {
            try {
                ...
                if (parseObserver2 != null) {
                    parseObserver2.parsed(bytes, i5, 0, "\nattributes[" + i6 + "]:\n");
                    this.observer.changeIndent(1);
                }
                Attribute parse = this.attributeFactory.parse(this.cf, this.context, i5, this.observer);
                i5 += parse.byteLength();
                this.list.set(i6, parse);
                ParseObserver parseObserver3 = this.observer;
                if (parseObserver3 != null) {
                    parseObserver3.changeIndent(-1);
                    this.observer.parsed(bytes, i5, 0, "end attributes[" + i6 + "]\n");
                }
            } catch (ParseException e5) {
                e5.addContext("...while parsing attributes[" + i6 + "]");
                throw e5;
