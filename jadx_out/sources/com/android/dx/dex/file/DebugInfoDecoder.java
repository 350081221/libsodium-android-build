...
public class DebugInfoDecoder {
    ...
    public static class LocalEntry {
        ...
        public LocalEntry(int i5, boolean z4, int i6, int i7, int i8, int i9) {
            this.address = i5;
            this.isStart = z4;
            this.reg = i6;
            this.nameIndex = i7;
    ...
    public static class PositionEntry {
        ...
        public PositionEntry(int i5, int i6) {
            this.address = i5;
            this.line = i6;
        }
    }

    DebugInfoDecoder(byte[] bArr, int i5, int i6, boolean z4, CstMethodRef cstMethodRef, DexFile dexFile) {
        int i7;
        if (bArr != null) {
            this.encoded = bArr;
            this.isStatic = z4;
            ...
            this.codesize = i5;
            this.lastEntryForReg = new LocalEntry[i6];
            try {
                i7 = dexFile.getStringIds().indexOf(new CstString("this"));
            } catch (IllegalArgumentException unused) {
                i7 = -1;
            }
            this.thisStringIdx = i7;
            return;
        }
    ...
    private void decode0() throws IOException {
        ...
        if (readUnsignedLeb128 == parameterTypes.size()) {
            ...
            while (true) {
                int readByte = byteArrayByteInput.readByte() & y1.f19838d;
                switch (readByte) {
                    case 0:
                        return;
                    case 1:
    ...
    public static void validateEncode(byte[] bArr, DexFile dexFile, CstMethodRef cstMethodRef, DalvCode dalvCode, boolean z4) {
        ...
        try {
            validateEncode0(bArr, insns.codeSize(), insns.getRegistersSize(), z4, cstMethodRef, dexFile, positions, locals);
        } catch (RuntimeException e5) {
            System.err.println("instructions:");
            insns.debugPrint((OutputStream) System.err, "  ", true);
            System.err.println("local list:");
            locals.debugPrint(System.err, "  ");
            throw ExceptionWithContext.withContext(e5, "while processing " + cstMethodRef.toHuman());
        }
    }

    private static void validateEncode0(byte[] bArr, int i5, int i6, boolean z4, CstMethodRef cstMethodRef, DexFile dexFile, PositionList positionList, LocalList localList) {
        boolean z5;
        LocalEntry localEntry;
        PositionEntry next;
        DebugInfoDecoder debugInfoDecoder = new DebugInfoDecoder(bArr, i5, i6, z4, cstMethodRef, dexFile);
        ...
        if (positionList2.size() == positionList.size()) {
            ...
            do {
                ...
                if (it.hasNext()) {
                    ...
                    int i7 = debugInfoDecoder.thisStringIdx;
                    int size2 = locals.size();
                    int paramBase = debugInfoDecoder.getParamBase();
                    for (int i8 = 0; i8 < size2; i8++) {
                        LocalEntry localEntry2 = locals.get(i8);
                        int i9 = localEntry2.nameIndex;
                        if (i9 < 0 || i9 == i7) {
                            int i10 = i8 + 1;
                            while (true) {
                                if (i10 < size2) {
                                    LocalEntry localEntry3 = locals.get(i10);
                                    if (localEntry3.address == 0) {
                                        if (localEntry2.reg == localEntry3.reg && localEntry3.isStart) {
                                            locals.set(i8, localEntry3);
                                            locals.remove(i10);
                                            size2--;
                                            break;
                                        }
                    ...
                    for (int i12 = 0; i12 < size3; i12++) {
                        ...
                        int i13 = localEntry.address;
                        if (localEntry.reg != entry2.getRegister()) {
                            System.err.println("local register mismatch at orig " + i12 + " / decoded " + i11);
                        } else if (localEntry.isStart != entry2.isStart()) {
                            System.err.println("local start/end mismatch at orig " + i12 + " / decoded " + i11);
                        } else if (i13 != entry2.getAddress() && (i13 != 0 || localEntry.reg < paramBase)) {
                            System.err.println("local address mismatch at orig " + i12 + " / decoded " + i11);
                        } else {
                            i11++;
                        }
                        z5 = true;
    ...
    public void decode() {
        try {
            ...
            throw ExceptionWithContext.withContext(e5, "...while decoding debug info");
        }
    }
