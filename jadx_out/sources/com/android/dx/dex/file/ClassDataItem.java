...
public final class ClassDataItem extends OffsettedItem {
    ...
    private static void encodeList(DexFile dexFile, AnnotatedOutput annotatedOutput, String str, ArrayList<? extends EncodedMember> arrayList) {
        ...
        for (int i6 = 0; i6 < size; i6++) {
            i5 = arrayList.get(i6).encode(dexFile, annotatedOutput, i5, i6);
        }
    }
    ...
    private static void encodeSize(DexFile dexFile, AnnotatedOutput annotatedOutput, String str, int i5) {
        if (annotatedOutput.annotates()) {
            annotatedOutput.annotate(String.format("  %-21s %08x", str + "_size:", Integer.valueOf(i5)));
        }
    ...
    public void debugPrint(Writer writer, boolean z4) {
        ...
        int size = this.staticFields.size();
        for (int i5 = 0; i5 < size; i5++) {
            printWriterFor.println("  sfields[" + i5 + "]: " + this.staticFields.get(i5));
        }
        int size2 = this.instanceFields.size();
        for (int i6 = 0; i6 < size2; i6++) {
            printWriterFor.println("  ifields[" + i6 + "]: " + this.instanceFields.get(i6));
        }
        int size3 = this.directMethods.size();
        for (int i7 = 0; i7 < size3; i7++) {
            printWriterFor.println("  dmeths[" + i7 + "]:");
            this.directMethods.get(i7).debugPrint(printWriterFor, z4);
        }
        int size4 = this.virtualMethods.size();
        for (int i8 = 0; i8 < size4; i8++) {
            printWriterFor.println("  vmeths[" + i8 + "]:");
            this.virtualMethods.get(i8).debugPrint(printWriterFor, z4);
        }
    }
