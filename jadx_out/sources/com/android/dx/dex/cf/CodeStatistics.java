...
public final class CodeStatistics {
    ...
    public void dumpStatistics(PrintStream printStream) {
        int i5 = this.runningDeltaInsns;
        printStream.printf("Optimizer Delta Rop Insns: %d total: %d (%.2f%%) Delta Registers: %d\n", Integer.valueOf(this.runningDeltaInsns), Integer.valueOf(this.runningTotalInsns), Double.valueOf((i5 / (this.runningTotalInsns + Math.abs(i5))) * 100.0d), Integer.valueOf(this.runningDeltaRegisters));
        int i6 = this.dexRunningDeltaInsns;
        printStream.printf("Optimizer Delta Dex Insns: Insns: %d total: %d (%.2f%%) Delta Registers: %d\n", Integer.valueOf(this.dexRunningDeltaInsns), Integer.valueOf(this.dexRunningTotalInsns), Double.valueOf((i6 / (this.dexRunningTotalInsns + Math.abs(i6))) * 100.0d), Integer.valueOf(this.dexRunningDeltaRegisters));
    ...
    public void updateOriginalByteCount(int i5) {
        this.runningOriginalBytes += i5;
    }
