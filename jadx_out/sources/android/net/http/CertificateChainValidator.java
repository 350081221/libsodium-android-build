...
public class CertificateChainValidator {
    ...
    private CertificateChainValidator() {
        try {
            ...
            throw new RuntimeException("X.509 TrustManagerFactory cannot be initialized", e5);
        } catch (NoSuchAlgorithmException e6) {
            throw new RuntimeException("X.509 TrustManagerFactory must be available", e6);
        }
    }
    ...
    public SslError doHandshakeAndValidateServerCertificates(HttpsConnection connection, SSLSocket sslSocket, String domain) throws IOException {
        ...
        return verifyServerDomainAndCertificates((X509Certificate[]) peerCertificates, domain, d.f20838a);
    }
    ...
    public static SslError verifyServerCertificates(byte[][] certChain, String domain, String authType) throws IOException {
        ...
        try {
            ...
            for (int i5 = 0; i5 < certChain.length; i5++) {
                serverCertificates[i5] = (X509Certificate) cf.generateCertificate(new ByteArrayInputStream(certChain[i5]));
            }
            return verifyServerDomainAndCertificates(serverCertificates, domain, authType);
        } catch (CertificateException e5) {
            throw new IOException("can't read certificate", e5);
        }
    }
    ...
    public static void handleTrustStorageUpdate() {
        try {
            ...
            Log.w(TAG, "Couldn't initialize default X.509 TrustManagerFactory", e6);
    ...
    private void closeSocketThrowException(SSLSocket socket2, String errorMessage, String defaultErrorMessage) throws IOException {
        closeSocketThrowException(socket2, errorMessage != null ? errorMessage : defaultErrorMessage);
    }

    private void closeSocketThrowException(SSLSocket socket2, String errorMessage) throws IOException {
        if (socket2 != null) {
            SSLSession session = socket2.getSession();
            if (session != null) {
                session.invalidate();
            }
            socket2.close();
