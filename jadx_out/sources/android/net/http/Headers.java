...
public final class Headers {
    ...
    public void getHeaders(HeaderCallback hcb) {
        for (int i5 = 0; i5 < 19; i5++) {
            String h5 = this.mHeaders[i5];
            if (h5 != null) {
                hcb.header(sHeaderNames[i5], h5);
            }
        }
        int extraLen = this.mExtraHeaderNames.size();
        for (int i6 = 0; i6 < extraLen; i6++) {
            hcb.header(this.mExtraHeaderNames.get(i6), this.mExtraHeaderValues.get(i6));
        }
    }
    ...
    static boolean containsIgnoreCaseTrimmed(CharArrayBuffer buffer, int beginIndex, String str) {
        ...
        boolean ok2 = ok;
        for (int j5 = 0; ok2 && j5 < size; j5++) {
            char a5 = chars[beginIndex + j5];
            char b5 = str.charAt(j5);
            if (a5 != b5) {
                ok2 = Character.toLowerCase(a5) == Character.toLowerCase(b5);
            }
        }
        return true;
    }
    ...
    static int setLowercaseIndexOf(CharArrayBuffer buffer, int ch) {
        ...
        for (int i5 = 0; i5 < endIndex; i5++) {
            char current = chars[i5];
            if (current == ch) {
                return i5;
            }
            chars[i5] = Character.toLowerCase(current);
        }
        return -1;
    }
}
